...
## 2026-08-24T06:35Z APPLIED — +3 / ~1 [opus-4.6 quality gate]

Heuristic runner: 1009 candidates discovered, 163 flagged create, 0 overwrite, 1 skip, 845 reject. Opus gate read the flagged set and overrode the heuristic almost entirely — of 163 "create" flags, 67 were genuine SKILL.md files and 96 were repo READMEs (awesome-lists, products, SEO-star repos). All three RSS feeds 403'd again (Medium x2, anthropic.com); GitHub search + curated repos supplied everything. No X/Exa keys, so those source classes were skipped.

Notable: `anthropics/skills` has moved substantially since our 2026-04-22 capture — `claude-api` alone grew from ~5K to ~75K chars and now documents a different API generation. Worth re-checking anthropics/skills monthly rather than treating April captures as current.

### Created (3)
- [create] `building-mcp-servers` score=90 (25/25/20/20/10) — rewritten from anthropics/skills `mcp-builder`. Source is a phase index that delegates content to reference/*.md files that don't transfer; rebuilt as a self-contained decision framework (tool granularity, stdio vs stateless streamable-HTTP, response projection/pagination), a TypeScript worked example with Zod + all four annotations, actionable-error rules, anti-patterns, and a 10-question read-only eval protocol. Net-new: the library had zero MCP coverage despite MCP being the default way to expose an API to Claude. Stack fit 25 (TS/Node + Python).
- [create] `co-authoring-technical-docs` score=85 (15/25/20/20/10) — rewritten from anthropics/skills `doc-coauthoring` (15.8K). Source is generic and connector-centric; retargeted at engineering decision docs with a concrete default section list, decision-first ordering, evidence discipline ("every claim gets a number, a link, or an explicit unknown"), and the blind-reader test as the load-bearing stage. Similarity to `building-rag-ready-docs` (retrieval structuring) and `documenting-api-contracts` (reference docs) checked and low — different artifact, different failure mode.
- [create] `evaluating-skill-performance` score=88 (15/25/20/20/8) — rewritten from the evaluation half of anthropics/skills `skill-creator` (33K). Deliberately narrowed to measurement + description tuning so it does not overlap `designing-workflow-skills` (structure) or `extracting-reusable-skills` (candidate identification). Kept the non-obvious parts: mandatory baseline arm spawned in the same turn, snapshot-before-edit, transcript review over output review, overfitting table, and the near-miss negative-query rule for the 20-query trigger set. Dropped viewer/tooling specifics that don't transfer.

### Overwritten (1)
- [overwrite] `claude-api` score=92 vs incumbent ~80 (15/25/20/20/12) — replaces the 2026-04-22 capture (5.4K body). Upstream now documents a materially different API generation: current model tier and exact ID strings (no date suffixes), adaptive thinking + effort levels with `budget_tokens` removed (400), assistant-prefill removal (400), sampling-param removal, compaction (`compact-2026-01-12`) vs context editing (`context-management-2025-06-27`) as distinct features, task budgets vs `max_tokens`, and Managed Agents as a third API surface. The incumbent predated all of it and would have actively misled on prefills and `budget_tokens`. Re-condensed 75K → 5.8K as a decision map; deliberately does not hardcode prices or a model table, deferring live figures to docs.claude.com so the entry ages gracefully. Name preserved per overwrite-not-duplicate rule.

### Notable skips/rejects (individually reviewed)
- [skip] anthropics/skills `skill-creator` body-authoring half — the structure/anatomy/progressive-disclosure guidance duplicates `designing-workflow-skills`. Only the eval half was extracted (see above).
- [skip] awesome-skills/code-review-skill (★1786) — read in full. Broad-but-shallow across 24 ecosystems; incumbent `generating-pr-reviews` already runs the same four-phase structure with React/Next, TS/Node and Python/FastAPI/Django checkpoints. Well under the 15% overwrite bar.
- [skip] nextlevelbuilder/ui-ux-pro-max-skill `ui-styling` and `design-system` — shadcn/Tailwind and token-architecture territory already held by `building-nextjs-shadcn-interfaces` and `enforcing-design-consistency`. `design-system` in particular is a manifest of reference files and scripts, not standalone content (depth ≈ 6 → reject on dimension 4 regardless of similarity).
- [skip] github/spec-kit, alirezarezvani `spec-driven-workflow` / `spec-to-repo` — duplicate of `spec-driven-development`.
- [reject] trailofbits `testing-handbook-generator` — again this cycle. Repo-specific generator bound to a cloned handbook and their plugin layout; unusable standalone.
- [reject] trailofbits `diagramming-code` — a CLI wrapper around the `trailmark` tool with bundled scripts. Interesting topic (Mermaid architecture diagrams from a parsed code graph) but the skill is a shell around a dependency that doesn't ship with us.
- [reject] trailofbits `modern-cpp`, `c-review`, `rust-review`, `writing-lean-proofs`, `address-sanitizer`, `yara-rule-authoring`, and the algorand/cairo/solana/substrate/ton scanners — off-stack, unchanged verdict from prior cycles.
- [skip] trailofbits `modern-python` — still the source of `configuring-modern-python-projects`; delta unchanged since 2026-08-11, still under the overwrite bar.
- [skip] trailofbits `semgrep-rule-creator` — incumbent body 7002 chars vs upstream 7008; effectively identical.
- [reject] alirezarezvani/claude-skills business + role tracks (capacity-planner, procurement-optimizer, vendor-management, chief-ai-officer-advisor, channel-economics, partnerships-architect, pricing-strategist/pricing-strategy, fda-consultant-specialist, risk-management-specialist, clinical-research, market-research, product-research, research-finance, ab-test-setup, churn-prevention, onboarding-cro, copy-editing, behuman) — off-stack for an engineering library.
- [reject] alirezarezvani persona skills (senior-fullstack, senior-devops, azure-cloud-architect, gcp-cloud-architect, red-team, adversarial-reviewer) — "act as a senior X" framing; either duplicate territory (`adversarial-code-review`) or role-play without decision content.
- [reject] alirezarezvani `codebase-onboarding` (2.2K) and `experiment-designer` (2.9K) — too thin; also adjacent to `cataloging-codebase-patterns`.
- [skip] alirezarezvani `chaos-engineering`, `docker-development`, `secrets-vault-manager`, `llm-cost-optimizer`, `handoff` — duplicate territory (`running-chaos-experiments`, `docker-optimization`/`shipping-containerized-services`, `managing-secrets-infrastructure`, `optimizing-llm-spend`, `writing-session-handoffs`).

### Bulk rejects (by class)
- ~45 awesome-lists and mega-repos surfaced by GitHub search (awesome-python, awesome-go, awesome-selfhosted, awesome-mcp-servers, free-for-dev, coding-interview-university, professional-programming) — link indexes with no skill content; the heuristic scores them 70–85 purely on README length and star count.
- ~30 products and templates misread as skills (dify, firecrawl, spec-kit, open-saas, html-anything, ai-website-cloner-template, skybridge, codepilot, moneyprinterturbo) — install-me repos.
- ~25 implausible-star repos (ecc ★242K, superpowers ★276K, ui-ux-pro-max-skill ★120K, gstack ★129K, ruview ★91K, agency-agents ★147K, anthropic-cybersecurity-skills ★30K) — star counts inconsistent with repo substance; rejected on content regardless of social score.
- Remainder: shallow READMEs under the 600-char useful-content floor, or duplicate-territory candidates below the uniqueness bar.

### Source-health note for a human
`hunter/sources.yml` RSS block has now failed three cycles running (403 on all three feeds — Medium and anthropic.com both block the default urllib User-Agent). Either set a browser UA in `cached_get` or drop the RSS block; as written it contributes nothing but log noise. Separately, `github_search_queries` has a poor signal ratio (96 of 163 flagged candidates were non-skill repos) — adding a `path:**/SKILL.md` code-search pass, or a curated-repo entry for `nextlevelbuilder/ui-ux-pro-max-skill` and `bitjaru/styleseed` (47 SKILL.md files, frontend-adjacent), would raise yield more than another repo-search query.
